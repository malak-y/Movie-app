...
export default function Navbar() {
  ...
  const menuHover =
    theme === "dark" ? "hover:bg-[#3a1e1f]" : "hover:bg-gray-200";
  const borderColor =
    theme === "dark" ? "border-[#4a2a2b]" : "border-gray-300";
  const buttonBg = theme === "dark" ? "bg-[#3a1e1f]" : "bg-gray-200";
  const buttonHover =
    theme === "dark" ? "hover:bg-[#4a2a2b]" : "hover:bg-gray-300";
  const buttonText = theme === "dark" ? "text-gray-300" : "text-black";

  return (
    <nav
      className={`${navBg} ${navText} px-6 py-4 flex items-center justify-between border-b ${borderColor} shadow-xl relative transition-colors duration-300`}
    >
      <div className="flex items-center space-x-4">
        <button
          className={`md:hidden ${navText} text-2xl p-2 rounded-lg ${buttonHover} transition-colors`}
        ...
        <Link to="/" className="flex items-center space-x-3 group">
          ...
          <div className="flex flex-col">
            <span
              className={`text-xl font-bold bg-clip-text text-transparent transition-colors duration-300 ${
